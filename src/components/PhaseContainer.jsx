...
import GamePlayer from "./Games/MemoryGame";
...
export default function PhaseContainer() {
  ...
  const renderPhaseContent = () => {
    switch (step) {
      case 1:
        return <WelcomeScreen onGetStarted={() => goToStep(step + 1)} />;
      case 2:
        return <AvatarSelection onDone={handleAvatarSelection} onAvatarSelect={setSelectedAvatar} logo={logo} />;
      case 3:
      ...
      case 11:
        return <GamePlayer onComplete={() => goToStep(step + 1)} />;
      case 12:
  ...
  return (
    <div style={{ minHeight: '100vh', width: '100%', position: 'relative', overflow: 'hidden', background: 'linear-gradient(180deg, #a8d5e2 0%, #e8f4f8 100%)' }}>
      {/* Only show header if step > 1 */}
      {step > 1 && (
        <div
          className="fixed-header mobile-dark-header"
         style={{ backgroundColor: 'rgba(13, 49, 43, 0.95)' }}
        >
          <div
            style={{
              width: '100%',
              ...
              backgroundColor: 'rgba(13, 49, 43, 1)'
            }}
          >
...
<div 
  style={{
    ...
    paddingTop: (step === 2 || step === 3 || step === 4 || step === 5 || step === 6 || step === 7 || step === 10 || step === 11 || step === 12 || step === 13) ? '0' : 'clamp(0.3rem, 1vw, 0.75rem)'
  }}
>
  <div style={{ 
    width: '100%', 
    maxWidth: '100%'  // 👈 CHANGED: Removed the conditional - always 100% width
  }}>
              ...
              >
                <div style={{ 
                  minHeight: (step === 2 || step === 3 || step === 4 || step === 5 || step === 6 || step === 7 || step === 10 || step === 11 || step === 12 || step === 13) ? 'auto' : 'clamp(150px, 25vh, 250px)', 
                  display: 'flex', 
                ...
                {(step !== 2 && step !== 3 && step !== 4 && step !== 5 && step !== 6 && step !== 7 && step !== 8 && step !== 10 && step !== 11 && step !== 12 && step !== 13) && (
                  <div style={{
                    display: 'flex',
                    justifyContent: 'space-between',
                    alignItems: 'center',
